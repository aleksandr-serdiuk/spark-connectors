package org.example.datasource.postgres_hw
...
class PostgresTable(val name: String) extends SupportsRead with SupportsWrite {
  def schema(): StructType = PostgresTable.schema
...
object PostgresPartitionsList {
  private def getTableBoundParams(connectionProperties: ConnectionProperties): (Int) = {
  ...
  def apply(connectionProperties: ConnectionProperties): Array[InputPartition] = {
    val cnt = getTableBoundParams(connectionProperties)
    val partitionSize = connectionProperties.partitionSize
    val partitions = ArrayBuffer[PostgresPartition]()

    for (i <- 0 to cnt by partitionSize) {
      val offset = i + partitionSize - partitionSize
      partitions +=
        new PostgresPartition(
          partitionSize,
          offset
        )
        //println(s"${partitionSize} - ${offset}")
    }
...
class PostgresPartition(val limit: Long,
